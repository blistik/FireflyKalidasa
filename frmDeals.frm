...
With sftTree
   ...
   If dealFilter = "bounties" Then
      Me.Caption = "Bounties"
   ElseIf Left(dealFilter, 5) = "local" Then
...
'         End If
   ...
   While Not rst3.EOF
      ...
      SQL = SQL & " ORDER BY Contact.ContactName,ContactDeck.Pay DESC, ContactDeck.CardID"
      ...
      While Not rst.EOF
         ...
         End If
         If rst3!ContactID = 10 Then
            .CellForeColor(Index, 1) = 14019306
         Else
         ...
         Case "UN" 'consider
            Select Case actionSeq
               ...
               Case ASDealSelect
                  ...
                  If max > 2 Then max = 2       'accept only up to 2 jobs, not exceeding 3 inactive in hand
                  If getSelected("R") < max Then
