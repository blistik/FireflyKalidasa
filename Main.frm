...
Begin VB.MDIForm Main 
   ...
   Begin VB.Timer tmrBeam 
      ...
      Top             =   1800
   End
   Begin VB.Timer tmrTokenAnim 
      Enabled         =   0   'False
      Interval        =   20
      Left            =   3600
      Top             =   1800
   End
   Begin VB.Timer tmrATokenAnim 
      Enabled         =   0   'False
      Interval        =   20
      Left            =   3030
      Top             =   1800
...
Private usedStitchSkill As Boolean, dontnagme As Boolean, rejoininprogress As Boolean, queuedExtraMoves As Integer

Private beamX1 As Long, beamY1 As Long, beamX2 As Long, beamY2 As Long
Private beamDX As Single, beamDY As Single
Private beamSector1 As Integer, beamSector2 As Integer
Private beamStep As Integer, beamStepsTotal As Integer
Private lineStep As Integer, lineStepsTotal As Integer, lineType As Integer

Private CryBabyDriftX As Single
Private CryBabyDriftY As Single
...
Dim X
   ...
   'read Dock & Float states from Computer\HKEY_CURRENT_USER\SOFTWARE\FireFly the PC Game
   
   DockFrame1.SetRegRootKey KeyCurentUser
   DockFrame1.LoadStates App.ProductName
...
Private Sub MDIForm_QueryUnload(Cancel As Integer, UnloadMode As Integer)
   'save Dock & Float states to Computer\HKEY_CURRENT_USER\SOFTWARE\FireFly the PC Game
   DockFrame1.SaveStates App.ProductName
...
Private Sub SaveMySettings()
   If player.PlayName <> "" Then
      SaveSetting App.ProductName, "Game", "PlayerName", player.PlayName
   End If
   SaveSetting App.ProductName, "Game", "AutoJobcard", Toolbar1.Buttons("help").ButtonMenus("viewjobcard").Tag
   
   If Not (frmDeal Is Nothing) Then
      If frmDeal.sftTree.ColumnWidth(3) > 2000 Then SaveSetting App.ProductName, "frmDeal", "Col3", CStr(frmDeal.sftTree.ColumnWidth(3))
   End If
   

End Sub

'THE MAIN ENGINE of this GAME - a figment of my imagination :^)
' Game States E - Idle/End, H - Host screen, L- Pick Leader, 1-4 players go.
' S - setup Game
' R - run Game, T-Trade
...
On Error GoTo err_handler
   ...
   If status <> "H" And status <> "E" And status <> "L" And pickStartSector > -1 And player.ID > 0 Then
      If Logic.Fields("Refresh" & player.ID) = 1 Then
         RefreshBoard
         refreshSolid
         clearRefresh
      End If
   ...
   ElseIf status = "S" And thisplayer = player.ID And pickStartSector = 0 Then  'your go to pick starting sector on MAP
      ...
      If StartSectorIDs <> "" Then  'use story driven starting sectors
         ...
         For X = 5 To 6 + NumOfReavers
            MoveShip X, IIf(Val(a(X - 1)) = 0, varDLookup("StartSectorID", "Players", "PlayerID=" & CStr(X)), Val(a(X - 1))), 0, False, True, 5000
         Next X
         If Val(a(player.ID - 1)) > 0 Then
            MoveShip player.ID, Val(a(player.ID - 1)), 0, False, True, 6000
      ...
      Else  'use the default setup params
         'set game ships
         For X = 5 To 6 + NumOfReavers
            MoveShip X, varDLookup("StartSectorID", "Players", "PlayerID=" & CStr(X)), 0, False, True, 5000
         Next X
   ...
   ElseIf status = "R" And thisplayer = player.ID And actionSeq = ASnavEnd Then   'fullburn Cycle
      ...
      If actionSeq = ASnavEnd Then 'has not been modified by special moves
         refreshSolid
         Verse.lblSolid(5).Visible = isSolid(player.ID, 5)
   ...
   ElseIf status = "R" And thisplayer = player.ID And actionSeq = ASWork Then   'Work Cycle - your go
      ...
      ElseIf Val(frmAction.lblJobName.Tag) = -2 Then 'Early's Datascope
         DrawDeck "Supply", SupplyID, 3
         frmBuy.RefreshBuys
         actionSeq = ASselect 'in limbo awaiting user to select
         
      ElseIf Val(frmAction.lblJobName.Tag) < -2 Then 'grab a Bounty
      ...
      Else  'do a Job
         ...
         End If
         LastJobDone = Val(frmAction.lblJobName.Tag)  'let any card open know of the update
         refreshSolid
         Verse.lblSolid(5).Visible = isSolid(player.ID, 5)
         'reset OffJob status
         clearOffJob player.ID
...
Private Sub tmrAnim_Timer()
   Dim i As Integer, p As Double, nowT As Double
   Dim xVal As Long, yVal As Long
   Dim jx As Long, jy As Long
   Const NoOfShips As Integer = 12

   nowT = Timer

   For i = 1 To NoOfShips
      If Anim(i).Active Then
         p = (nowT - Anim(i).startTime) * 1000 / Anim(i).durationMs
         If p >= 1 Then p = 1
         If p < 0 Then p = 0
      
         xVal = Anim(i).StartX + (Anim(i).targetX - Anim(i).StartX) * p
         yVal = Anim(i).StartY + (Anim(i).targetY - Anim(i).StartY) * p
      
         ' Reaver-only jitter DURING movement only
         If i > 6 And p < 1 Then
            jx = Int(40 * Rnd + -20)
            jy = Int(40 * Rnd + -20)
         Else
...
Public Sub MoveShip_AnimationComplete(playerID As Integer)
    
    With Verse.Imag(playerID)
        .Mirror = lvicMirrorNone
        .Animate2.StartAnimation
        .Refresh
        If playerID = 5 Then MoveSolid Verse.lblSolid(5), 5
    End With
    'catch if extramoves are needed
    If playerID = player.ID And queuedExtraMoves > 0 Then
        moveAutoAI player.ID, 1
        drawLine 0, -2, getPlayerSector(player.ID)
        drawLine 1, -2, getPlayerSector(player.ID)
        queuedExtraMoves = queuedExtraMoves - 1
    End If
...
Private Sub tmrBeam_Timer()
   ...
   If beamStep > -1 Then
      beamStep = beamStep + 1
      Verse.Beam.Left = Verse.Beam.Left + beamDX
      Verse.Beam.Top = Verse.Beam.Top + beamDY
   End If
   
   If lineType > 2 And beamStep >= beamStepsTotal Then
      ' Beam finished — caller decides what to do next
      beamStep = -1
      ' impact moment
      Verse.Beam.Visible = False
      tmrBeam.Enabled = False
      moveAutoAI lineType
     
   ElseIf beamStep >= beamStepsTotal Then
      ' Beam finished — caller decides what to do next
      beamStep = -1  'flag the wormhole has opened, now show the solid beam
      lineStep = 0
      
      Main.drawLine lineType, beamSector2, beamSector1
   ElseIf beamStep = -1 Then 'animate the line width
      lineStep = lineStep + 1
      Verse.Beam.Width = Abs(Verse.Beam.Width - (lineStep * 7))
      Verse.Beam.Height = Abs(Verse.Beam.Height - (lineStep * 7))
      Verse.LineB(lineType).BorderWidth = Verse.LineB(lineType).BorderWidth - 1
      If lineStep >= lineStepsTotal Then
         Verse.Beam.Visible = False
         tmrBeam.Enabled = False
         playsnd 2
      End If
...
'mode 0-own job, 1-dealer job, 2-wormhole, 7+ - flakgun
Public Sub startBeam(ByVal sector1 As Integer, ByVal sector2 As Integer, ByVal mode As Integer)

    Dim rst As New ADODB.Recordset
    Dim SQL As String, X, Y
    
    If sector1 = 1 Then
       sector1 = getCruiserSector()
    ElseIf sector1 = 2 Then
       sector1 = getCorvetteSector()
    ElseIf sector1 = 3 Then
       Exit Sub
    End If
    
    If mode < 2 Then
       If Verse.LineB(mode).Visible = True And sector1 = beamSector2 Then
         Verse.LineB(mode).Visible = False
         Exit Sub
       End If
    End If
    
    lineType = mode 'line array index
    
    If mode > 2 Then
       beamX1 = Verse.Imag(player.ID).Left + Int(Verse.Imag(player.ID).Width / 2)
       beamY1 = Verse.Imag(player.ID).Top + Int(Verse.Imag(player.ID).Height / 2)
       beamX2 = Verse.Imag(mode).Left + Int(Verse.Imag(mode).Width / 2)
       beamY2 = Verse.Imag(mode).Top + Int(Verse.Imag(mode).Height / 2)
    
    Else
    
       'see which one our ship is closect to
       X = getSectorCount(getPlayerSector(player.ID), sector1)
       Y = getSectorCount(getPlayerSector(player.ID), sector2)
       If X > Y Or mode < 2 Then 'swap them
          X = sector2
          sector2 = sector1
          sector1 = X
       End If
       ...
       ' --- Lookup sector1 coords ---
       SQL = "SELECT * FROM Board WHERE SectorID=" & sector1
       rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
       If Not rst.EOF Then
           beamX1 = rst!SLeft + Int(rst!SWidth / 2)
           beamY1 = rst!STop + Int(rst!SHeight / 2)
       End If
       rst.Close
   
       ' --- Lookup sector2 coords ---
       SQL = "SELECT * FROM Board WHERE SectorID=" & sector2
       rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
       If Not rst.EOF Then
           beamX2 = rst!SLeft + Int(rst!SWidth / 2)
           beamY2 = rst!STop + Int(rst!SHeight / 2)
       End If
       rst.Close
    End If
   
    ' --- Position the beam sprite ---
    With Verse.Beam
      Select Case mode
      Case Is > 2
         .Picture = LoadPictureGDIplus(App.Path & "\Pictures\flak.gif")
         .Width = 240
         .Height = 240
         beamStepsTotal = 22
         tmrBeam.Interval = 20
      Case 2
         Verse.LineB(lineType).Visible = False
         .Picture = LoadPictureGDIplus(App.Path & "\Pictures\wormhole.gif")
         .Width = 720
         .Height = 720
         Verse.LineB(lineType).BorderWidth = 40
         beamStepsTotal = 120
         lineStepsTotal = 37
         tmrBeam.Interval = 40
      Case 0, 1
         Verse.LineB(lineType).Visible = False
         .Picture = LoadPictureGDIplus(App.Path & "\Pictures\radar.gif")
         .Width = 400
         .Height = 400
         Verse.LineB(lineType).BorderWidth = 20
         beamStepsTotal = 40
         lineStepsTotal = 17
         tmrBeam.Interval = 26
      End Select
    ...
    beamDX = (beamX2 - beamX1) / beamStepsTotal
    beamDY = (beamY2 - beamY1) / beamStepsTotal
...
Private Sub tmrCryBaby_Timer()

   ' Exponential drift (accelerate downwards)
   CryBabyDriftX = CryBabyDriftX * 1.07
   CryBabyDriftY = CryBabyDriftY * 1.06
   ...
   With Verse.CryBaby
      ...
      ' Re-center while drifting
      .Left = .Left - 20 + CryBabyDriftX
      .Top = .Top + CryBabyDriftY
           
      ' Rotate
      If .Rotation > 359 Then .Rotation = 0
      .Rotation = .Rotation + 1   ' Spin speed (1 degrees per tick)
   
      ' Fade out
      If .Width > 6000 Then
         .TransparencyPct = .TransparencyPct + 1
      End If
      
      ' End condition
      If .Top > Verse.Height Then
          tmrCryBaby.Enabled = False
          .Visible = False
          moveAutoAI 5
      End If
   End With
...
Private Sub tmrTokenAnim_Timer()
   Dim i As Integer
   Dim nowT As Double
   Dim p As Double
   Dim xVal As Long
   ...
   For i = 1 To NO_OF_SECTORS
      If TokenAnim(i).Active Then
          done = False
      
          ' Progress 0 ? 1
          p = (nowT - TokenAnim(i).startTime) * 1000 / TokenAnim(i).durationMs
          If p >= 1 Then p = 1
          If p < 0 Then p = 0
      
          ' Reaver-style easing (slightly chaotic)
          ' Faster start, slight overshoot feel
          p = p ^ 0.85
      
          ' Interpolate position
          xVal = TokenAnim(i).StartX + (TokenAnim(i).targetX - TokenAnim(i).StartX) * p
          yVal = TokenAnim(i).StartY + (TokenAnim(i).targetY - TokenAnim(i).StartY) * p
          jx = Int(40 * Rnd + -20)    ' -3 to +3 px
          jy = Int(40 * Rnd + -20)
          With Main.Verse.imgToken(i)
              .Left = xVal + jx
              .Top = yVal + jy
              .Refresh
          End With
         ...
         ' Arrival
         If p >= 1 Then
            TokenAnim(i).Active = False
            
            ' Start pulse
            TokenAnim(i).PulseActive = True
            TokenAnim(i).PulseStartTime = nowT
            TokenAnim(i).PulseDurationMs = 300   ' sharper than Alliance
            
            ' Store original size for non-compounding scale
            TokenAnim(i).OrigW = Main.Verse.imgToken(i).Width
            TokenAnim(i).OrigH = Main.Verse.imgToken(i).Height
         End If
      
      End If
      
      ' Pulse block
      If TokenAnim(i).PulseActive Then
         done = False
      
         p = (nowT - TokenAnim(i).PulseStartTime) * 1000 / TokenAnim(i).PulseDurationMs
         If p >= 1 Then p = 1
         If p < 0 Then p = 0
      
         ' Reaver pulse: sharp, violent, quick
         Dim pulseScale As Double
         pulseScale = 1 + (0.25 * (1 - (2 * Abs(p - 0.5))))   ' spike at midpoint
         ...
         With Main.Verse.imgToken(i)
            ...
            ' Apply scale relative to original size (no compounding)
            .Aspect = lvicScaled
            .Width = TokenAnim(i).OrigW * pulseScale
            .Height = TokenAnim(i).OrigH * pulseScale
   ...
   If done Then
       Me.tmrTokenAnim.Enabled = False
   End If
End Sub

Private Sub tmrATokenAnim_Timer()
    Dim i As Integer
    Dim nowT As Double
    Dim p As Double, p2 As Double, pp As Double
    Dim xVal As Double, yVal As Double
    Dim launchStartY As Double
    Dim sngScale As Double
    Dim newW As Double, newH As Double
    Dim pulseScale As Double
    Dim pw As Double, ph As Double, pc As Double
    Dim anyActive As Boolean

    Const JETTISON_FRACTION As Double = 0.5
    Const JETTISON_OFFSET As Long = 400          ' TWIPS
    Const START_SCALE As Double = 0.2
    Const END_SCALE As Double = 1#
    Const TOKEN_BASE_W As Long = 705             ' 45px ? 675 twips
    Const TOKEN_BASE_H As Long = 705
    Const PULSE_ENLARGE As Double = 0.35   ' 35% enlargement
    ...
    For i = 1 To NO_OF_SECTORS
         ...
         ' ============================================================
         If ATokenAnim(i).Active Then
             anyActive = True
         
             ' Normalized progress 0 ? 1
             p = (nowT - ATokenAnim(i).startTime) * 1000# / ATokenAnim(i).durationMs
             If p < 0 Then p = 0
             If p > 1 Then p = 1
         
             ' -----------------------------
             ' Position interpolation (TWIPS)
             ' -----------------------------
             If p < JETTISON_FRACTION Then
                 ' Phase 1: jettison downward
                 xVal = ATokenAnim(i).StartX
                 yVal = ATokenAnim(i).StartY + (JETTISON_OFFSET * (p / JETTISON_FRACTION))
             Else
                 ' Phase 2: launch toward target
                 p2 = (p - JETTISON_FRACTION) / (1 - JETTISON_FRACTION)
                 If p2 < 0 Then p2 = 0
                 If p2 > 1 Then p2 = 1
         
                 launchStartY = ATokenAnim(i).StartY + JETTISON_OFFSET
         
                 xVal = ATokenAnim(i).StartX + (ATokenAnim(i).targetX - ATokenAnim(i).StartX) * p2
                 yVal = launchStartY + (ATokenAnim(i).targetY - launchStartY) * p2
             End If
         
             ' -----------------------------
             ' Scale interpolation
             ' -----------------------------
             sngScale = START_SCALE + (END_SCALE - START_SCALE) * p
             newW = TOKEN_BASE_W * sngScale
             newH = TOKEN_BASE_H * sngScale
         
             ' -----------------------------
             ' Apply to control (TWIPS)
             ' -----------------------------
             With Verse.imgAToken(i)
                 .Width = newW
                 .Height = newH
                 .Left = xVal
                 .Top = yVal
             ...
             ' -----------------------------
             If p >= 1 Then
                 ATokenAnim(i).Active = False
         
                 ' Trigger pulse
                 ATokenAnim(i).PulseActive = True
                 ATokenAnim(i).PulseStartTime = nowT
                 ATokenAnim(i).PulseDurationMs = 500
             End If
         ...
         ' ============================================================
         If ATokenAnim(i).PulseActive Then
             anyActive = True
         
             pp = (nowT - ATokenAnim(i).PulseStartTime) * 1000# / ATokenAnim(i).PulseDurationMs
             If pp < 0 Then pp = 0
             If pp > 1 Then pp = 1
         
             ' Pulse curve: 1.0 -> (1.0 + PULSE_ENLARGE) -> 1.0
             If pp < 0.5 Then
                 pc = 1# + (PULSE_ENLARGE * (pp / 0.5))          ' grow
             Else
                 pc = (1# + PULSE_ENLARGE) - (PULSE_ENLARGE * ((pp - 0.5) / 0.5))   ' shrink
             End If

             'pc = 1# + (PULSE_ENLARGE * pp) ' TEMP: grow only
             ...
             With Verse.imgAToken(i)
                 ...
                 ' Center pulse around final top-left
                 .Left = ATokenAnim(i).targetX - (pw - TOKEN_BASE_W) / 2
                 .Top = ATokenAnim(i).targetY - (ph - TOKEN_BASE_H) / 2
         
                 .Refresh
             End With
         
             If pp >= 1 Then
                 ATokenAnim(i).PulseActive = False
                 ...
                 With Verse.imgAToken(i)
                     ...
                     .Width = TOKEN_BASE_W
                     .Height = TOKEN_BASE_H
                     .Left = ATokenAnim(i).targetX
                     .Top = ATokenAnim(i).targetY
                 End With
             End If
         End If
    ...
    ' ============================================================
    ' SHUT DOWN TIMER IF NOTHING IS ACTIVE
    ' ============================================================
    If Not anyActive Then
        tmrATokenAnim.Enabled = False
    End If
  ...
  Case "start"  'host
    ...
    Select Case Logic!Seq
     Case "E"
       ...
       player.Color = ""
       player.PlayName = ""
       DB.Execute "UPDATE GameSeq SET Seq = 'H', GameCntr = 0, SecretSectorID = 0, SecretRevealed = 0"
       ClearBoard
       refreshSolid
       Starter.isHost = True
       Starter.Show 1
...
Dim coords, c() As String, SectorID As Integer
 playsnd 18
 With Verse
   ...
   While Not rst.EOF  'no other players
      SectorID = Nz(rst!SectorID, 0)
      
      displayShip rst!playerID, SectorID
   ...
   While Not rst.EOF
      'reaver code block moved to its own routine so as to be called to force a single tokens update
      displayToken rst!SectorID, rst!Token
      
      displayAToken rst!SectorID, rst!Atoken

      displayHaven rst!SectorID, rst!Haven
...
Public Sub displayToken(ByVal SectorID As Integer, ByVal Token As Integer)
Dim coords As String, c() As String
Dim oldCount As Integer

    With Verse
        
        oldCount = Val(.imgToken(SectorID).Tag)

        ' If token is being added, animate it
        If Token > oldCount Then
            
            '121   (Reaver Space)
            
            ' Animate from Reaver Space (Sector 121 Slot5 coords)
            coords = varDLookup("Slot5", "Board", "SectorID=121")
            c = Split(coords, ",")

            ' Animate from origin to this sector
            StartTokenMove SectorID, Val(c(0)), Val(c(1)), .imgToken(SectorID).Left, .imgToken(SectorID).Top, 3000

        End If

        ' Now update the static token image
        If Token = 0 Then
            .imgToken(SectorID).Visible = False
        ElseIf oldCount <> Token Then
            .imgToken(SectorID).Picture = LoadPictureGDIplus(App.Path & "\Pictures\RToken" & IIf(Token > 6, 6, Token) & ".gif")
            .imgToken(SectorID).Visible = True
            .imgToken(SectorID).TransparentColor = &HFFFFFF
            .imgToken(SectorID).TransparentColorMode = lvicUseTransparentColor
            .imgToken(SectorID).Animate2.StartAnimation
        End If

        .imgToken(SectorID).Tag = CStr(Token)
    End With
End Sub
...
'      If Token = 0 Then
'         .imgToken(sectorID).Visible = False
'      ElseIf Val(.imgToken(sectorID).Tag) <> Token Then
'         .imgToken(sectorID).Picture = LoadPictureGDIplus(App.Path & "\Pictures\RToken" & IIf(Token > 6, 6, Token) & ".gif")
'         .imgToken(sectorID).Visible = True
'         .imgToken(sectorID).TransparentColor = &HFFFFFF
'         .imgToken(sectorID).TransparentColorMode = lvicUseTransparentColor
...
Public Sub displayAToken(ByVal SectorID As Integer, ByVal Atoken As Integer)
Dim rst As New ADODB.Recordset
Dim cruiserSectorID As Integer
Dim coords As String
Dim c() As String
    With Verse
        Dim oldCount As Integer
        oldCount = Val(.imgAToken(SectorID).Tag)

        ' If token is being added, animate it
        If Atoken > oldCount Then
            
            cruiserSectorID = getCruiserSector()

            ' Get the Cruiser's Slot5 coords (same as your existing animation logic)
            
            rst.Open "SELECT Slot5 FROM Board WHERE SectorID = " & cruiserSectorID, DB, adOpenForwardOnly, adLockReadOnly
            ...
            If Not rst.EOF Then
                ...
                ' Animate from Cruiser to this sector
                StartATokenMove SectorID, Val(c(0)), Val(c(1)), .imgAToken(SectorID).Left, .imgAToken(SectorID).Top, 3000
            End If
        ...
        ' Now update the static token image
        If Atoken = 0 Then
            .imgAToken(SectorID).Visible = False
        ElseIf oldCount <> Atoken Then
            .imgAToken(SectorID).Picture = LoadPictureGDIplus(App.Path & "\Pictures\AToken" & IIf(Atoken > 6, 6, Atoken) & ".gif")
            .imgAToken(SectorID).Visible = True
            .imgAToken(SectorID).TransparentColor = &HFFFFFF
            .imgAToken(SectorID).TransparentColorMode = lvicUseTransparentColor
            .imgAToken(SectorID).Animate2.StartAnimation
        End If

        .imgAToken(SectorID).Tag = CStr(Atoken)
    End With
End Sub
...
'      If AToken = 0 Then
'         .imgAToken(sectorID).Visible = False
'      ElseIf Val(.imgAToken(sectorID).Tag) <> AToken Then
'         .imgAToken(sectorID).Picture = LoadPictureGDIplus(App.Path & "\Pictures\AToken" & IIf(AToken > 6, 6, AToken) & ".gif")
'         .imgAToken(sectorID).Visible = True
'         .imgAToken(sectorID).TransparentColor = &HFFFFFF
'         .imgAToken(sectorID).TransparentColorMode = lvicUseTransparentColor
...
Public Sub displayHaven(ByVal SectorID As Integer, ByVal Haven As Integer)
Dim coords, c() As String
   With Verse
      If Haven > 0 Then
         coords = varDLookup("Slot" & Haven, "Board", "SectorID = " & SectorID)
         c = Split(coords, ",")
         .imgHaven(Haven).Left = c(0)
         .imgHaven(Haven).Top = c(1)
         .imgHaven(Haven).Picture = LoadPictureGDIplus(App.Path & "\Pictures\Haven" & Haven & ".bmp")
         .imgHaven(Haven).Visible = True
         .imgHaven(Haven).TransparentColor = &HFFFFFF
         .imgHaven(Haven).TransparentColorMode = lvicUseTransparentColor
      End If
   End With
End Sub
...
Dim rst As New ADODB.Recordset
Dim coords, slot, X, a, B
Dim c() As String
   
   If SectorID = 0 Then  'remove
      Verse.Imag(playerID).Visible = False
   Else
      ...
      If Not rst.EOF Then
         ...
         c = Split(coords, ",")
         a = Verse.Imag(playerID).Left
         B = Verse.Imag(playerID).Top

         Verse.Imag(playerID).Visible = True
         
         'Reaver-Flage?
         If playerID > 0 And playerID < 5 Then
            ReaverFlageActive(playerID) = hasShipUpgrade(playerID, 18)
         End If
         
         If Not (a = Val(c(0)) And B = Val(c(1))) Then 'move
            ...
            Case Else   'cutters
               playsnd 3
            End Select
            If a < Val(c(0)) Then Verse.Imag(playerID).Mirror = lvicMirrorHorizontal
            StartMove playerID, a, B, Val(c(0)), Val(c(1)), 2000
         End If
      End If
   End If
...
Private Sub initToolbar(ByVal start As Boolean, Optional ByVal admin As Boolean = True)
   With Toolbar1
      ...
      .Buttons("help").ButtonMenus("viewjobcard").Tag = GetSetting(App.ProductName, "Game", "AutoJobcard", vbNullString)
      If .Buttons("help").ButtonMenus("viewjobcard").Tag = "" Then
         .Buttons("help").ButtonMenus("viewjobcard").Text = "Auto View Jobcard on Deal (Off)"
         .Buttons("help").ButtonMenus("viewjobcard").Tag = ""
      Else
         .Buttons("help").ButtonMenus("viewjobcard").Text = "Auto View Jobcard on Deal (On)"
         .Buttons("help").ButtonMenus("viewjobcard").Tag = "X"
      End If
   End With
End Sub
...
Dim frmJobEdit As frmJobEditor, X
   ...
   Case "viewjobcard"
      If Toolbar1.Buttons("help").ButtonMenus("viewjobcard").Tag = "" Then
         Toolbar1.Buttons("help").ButtonMenus("viewjobcard").Text = "Auto View Jobcard on Deal (On)"
         Toolbar1.Buttons("help").ButtonMenus("viewjobcard").Tag = "X"
      Else
         Toolbar1.Buttons("help").ButtonMenus("viewjobcard").Text = "Auto View Jobcard on Deal (Off)"
         Toolbar1.Buttons("help").ButtonMenus("viewjobcard").Tag = ""
      End If
   ...
   Case "about"
      MessBox "Firefly + Blue Sun/Kalidasa  V" & App.Major & "." & App.Minor & "." & App.Revision & vbNewLine & "*Open Freeware* - use at your own risk" & vbNewLine & "Coded by: Vee Bee-er " & Chr(169) & " 2021-25 BLiSoftware" & _
...
Dim rst As New ADODB.Recordset
   ...
   With frmNav
      ...
      'SectorID = Nz(getPlayerSector(player.ID), 0)
   ...
   With frmAction
      ...
      If (Not .buydone) And (Not onlyFullburn) And Not reaverActive Then  ' Buy and Shore leave *may* be active
         ...
         ElseIf getHaven(SectorID) > 0 And CargoCapacity(player.ID) - CargoSpaceUsed(player.ID) > 0 Then
            'Enabled
         ElseIf unseenDeck = 0 And Val(.imgSupply.Tag) > 0 And (actionSeq = ASselect Or actionSeq = ASBuySelDiscard Or actionSeq = ASBuySelect) Then  'modify to Consider
      ...
      unseenDeck = getUnseenDeck("Contact", Val(.imgContact.Tag))
      If .imgContact.Tag <> "" And actionSeq = ASDealSelDiscard And unseenDeck = 0 And (Not .dealdone) And (Not onlyFullburn) And Not reaverActive And Not (Val(.imgContact.Tag) = 5 And varDLookup("Warrants", "Players", "PlayerID=" & player.ID) > 0) Then
         .setMultiStateButton "imgDealer", "2a"
      ElseIf Not (.imgContact.Tag <> "" And (actionSeq = ASselect Or (actionSeq = ASDealSelDiscard And unseenDeck > 0) Or actionSeq = ASDealSelect) And (Not .dealdone) And (Not onlyFullburn) And Not reaverActive And Not (Val(.imgContact.Tag) = 5 And varDLookup("Warrants", "Players", "PlayerID=" & player.ID) > 0)) Then
      ...
      .setVisState .imgClearWarrants, ((varDLookup("Warrants", "Players", "PlayerID=" & player.ID) > 0) And (Val(.imgContact.Tag) = 2) And isSolid(player.ID, 2) And (Val(.lblCash.Tag) >= 1000) And Not HigginsDealPerk)
      ...
      SQL = SQL & "Where PlayerJobs.PlayerID = " & player.ID & " And (Job.SectorID IN (1,2,3," & SectorID & ") Or Job_1.SectorID IN (1,2,3," & SectorID & ") Or Job_2.SectorID IN (1,2,3," & SectorID & "))"
      rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
      '.cbo.Clear
      ...
      While Not rst.EOF
         ...
         If (isAtLocation(rst!sector1, SectorID)) And rst!JobStatus = 0 And getPlayerJobs(player.ID, "1,2") < MAXACTIVEJOBS + IIf(isSolid(player.ID, 8), 1, 0) Then ' check requirements met for job
         ...
         ElseIf (isAtLocation(rst!sector3, SectorID)) And rst!JobStatus = 1 Then 'Job3 must be in the sector
         ...
         ElseIf (isAtLocation(rst!sector2, SectorID)) And (rst!JobStatus = 1 Or rst!JobStatus = 2) Then 'Job2 must be in the sector
      ...
      'Early's Datascope?
      If hasGear(player.ID, 62) And Val(.imgSupply.Tag) > 0 Then
         X = X + 1
         If .mnuWorkPop.count < X + 1 Then Load .mnuWorkPop(X)
         .mnuWorkPop(X).Caption = "Reveal the top 3 Supply Cards"
         .mnuWorkPop(X).Tag = "-2"
      End If
      ...
      Else
         .lblJobName = ""
         .lblJobName.ToolTipText = "no work available"
         .lblJobName.Tag = ""
      End If
      ...
      .setVisState .imgLoadPassngr, (SectorID = 23) And isSolid(player.ID, 1) And .imgDealer.Tag <> "N" And (CargoCapacity(player.ID) - CargoSpaceUsed(player.ID) > 0.6) And Not HigginsDealPerk
      .setVisState .imgLoadFugi, .imgLoadPassngr.Tag = "Y" And Not HigginsDealPerk
...
Public Sub RefreshDeals()
   If Not (frmDeal Is Nothing) And actionSeq <> ASDealSelect And actionSeq <> ASDealSelDiscard Then
      If frmDeal.FDPane1.PaneVisible Then frmDeal.RefreshDeals
   End If
End Sub
...
Dim rst As New ADODB.Recordset, X, Y, parts As Integer, a() As String, DoubleDown As Integer
...
Dim frmSalvage As frmSalvaging, frmKillCrw As frmKillCrew, frmGamb As frmGamble, solidMsg As String
   ...
   rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
   If Not rst.EOF Then
      If rst!JobStatus = 0 And (isAtLocation(rst!sector1, SectorID)) Then ' we're doing Job 1
      ...
      ElseIf rst!JobStatus = 1 And (isAtLocation(rst!sector3, SectorID)) And Not IsNull(rst!Job3) Then ' we're doing Bonus Job
         JobID = rst!Job3
         bonus = rst!bonus
         finalstate = 2
         
      ElseIf (rst!JobStatus = 1 Or rst!JobStatus = 2) And (isAtLocation(rst!sector2, SectorID)) And Not IsNull(rst!Job2) Then  ' we're doing Job 2
         JobID = rst!Job2
         finalstate = JOB_SUCCESS
   ...
   ElseIf finalstate = JOB_SUCCESS Then 'job is ending, but do any remaining challenges Negotiate Pay or Cover your Tracks, Gamble
      ...
      If Not rst.EOF Then
         ...
         End If
         'KEYWORD CHECKS - ========================
         If rst!WinOptKeyword > 0 And hasKeyword(playerID, Nz(rst!KeyWords)) Then ' give option to use Keyword to Win -  only applies to HACKINGRIG or Explosives (getting paid half)
         ...
         'check Story for issuing a Warrant for an illegal Job
         If varDLookup("WarrantJob", "Story", "StoryID=" & Logic!StoryID) > 0 And rst!illegal > 0 Then
            DB.Execute "UPDATE Players Set Warrants = Warrants + 1, Solid5 = 0 WHERE PlayerID=" & player.ID
            PutMsg "Warrant issued to " & player.PlayName & " for this Illegal Job", playerID, Logic!Gamecntr
         End If
         
         '<<<< Get Paid, with any Bonus, less deductions & Go Solid with this Contact >>>>
   ...
   If ContactID = 10 Then
      Y = totalMisbehaves(player.ID, X)
      Y = countBounties(player.ID)
      frmAction.lblBounties = Y & IIf(X > 0, "/" & X, "")
      If X > 0 And Y >= X Then
         frmAction.lblBounties.ForeColor = &H3E631
      Else
         frmAction.lblBounties.ForeColor = &H3DCBFF
      End If
   ...
   If Not rst.EOF Then
      ...
      Case 0 ' winners are grinners :D
         ...
         ElseIf rst!WinFuel = 14 Then ' all you can load
            ...
            Do
               X = InputBoxx("Select up to " & skillcnt & " Fuel to salvage", "Salvage Fuel", CStr(skillcnt), getLeader())
         ...
         ElseIf rst!WinParts <> 0 Then ' could be -neg  . skillcnt re-used to count spare room for parts here
            skillcnt = (CargoCapacity(player.ID) - CargoSpaceUsed(player.ID)) * 2
         ...
         If rst!WinKillCrew <> 0 Then
            ...
            If rst!OptionName = "If we're very lucky" And hasShipUpgrade(player.ID, 18) > 0 And X > 0 Then
               ReaverFlageActive(player.ID) = False
      ...
      If rst!MovePlayer > 0 Then
            ...
            drawLine 0, -2, getPlayerSector(player.ID)
            drawLine 1, -2, getPlayerSector(player.ID)
            queuedExtraMoves = rst!MovePlayer - 1
      End If
      ...
      If rst!Warrant = 1 Or (result = 2 And rst!Warrant = 2) Or (isOutlaw(player.ID) And rst!Warrant = 3) Then
         If Not warrantDodge(player.ID) Then
            ...
            X = varDLookup("Warrants", "Players", "PlayerID =" & player.ID)
            frmAction.setVisState frmAction.imgWarrants, True
            frmAction.lblWarrants = X
            frmAction.lblWarrants.Visible = (X > 0)
         End If
      End If
...
Public Function doDeal(ByVal playerID As Integer, ByVal ContactID As Integer) As Integer
   ...
   With frmDeal.sftTree
      ...
      For Index = 0 To .ListCount - 1
         ...
         Case "R"  'selected
            ...
            If Toolbar1.Buttons("help").ButtonMenus("viewjobcard").Tag = "X" Then
               viewJobCard ContactID, .ItemData(Index)
            End If
   ...
   With frmBuy.sftTree
      ...
      For Index = 0 To .ListCount - 1
         ...
         Case "R"  'selected -pay up!!
            ...
            If .CellItemData(Index, 1) = 18 Then ReaverFlageActive(player.ID) = True
...
Dim X, G
   ...
   If X > 0 Then  'we got company!
      ...
      If G > 0 Then 'Flac Gun
         If MessBox("Reaver within firing Range, do you want to use the single-use Flac Gun to fend it off?", "Reaver Cutter", "FIRE!", "Hold fire", 0, 0, 15) = 0 Then
            playsnd 19
            'shoot the Flak Gun at the Cutter
            startBeam 0, 0, X
            checkFlacGun = True
...
Dim Havens As Boolean
   ...
   If pickStartSector = 1 Then
      ...
      If Not CheckClash(player.ID, Index, Havens) Then
         If Havens Then
            placeHaven player.ID, Index
            displayHaven Index, player.ID
         End If
   ...
   If actionSeq = ASResolveAlert Then
      If isAdjacent(player.ID, Index) And hasAlert(Index) Then
         playsnd 14
...
Private Sub Verse_SectDblClick(ByVal Index As Integer)
Dim msg, t, a
Dim frmD As frmDash

   t = varDLookup("Token", "Board", "SectorID=" & Index)
   a = varDLookup("AToken", "Board", "SectorID=" & Index)
   Set frmD = New frmDash
   With frmD
      .SectorID = Index
      .Atoken = a
      .Token = t
      .PlanetName = IIf(Index = UNKNOWN_SECTOR, "", getPlanetName(Index))
      .Distance = getSectorCount(getPlayerSector(player.ID), Index)
      .Show 0, Me
   End With
...
Dim SQL, X1, X2, Y1, Y2
   ...
   ElseIf sector1 = 3 Then
      Logic.Requery
      If Logic!SecretRevealed = 0 Or Logic!SecretSectorID = 0 Then
         Exit Sub
      Else
         sector1 = Logic!SecretSectorID
      End If
   End If
...
Dim CrewID, SupplyID, CrewCardID, killCrew, ShipID, JumpShipID
   ...
   End If
   LastJobDone = CardID  'let any card open know of the update
...
Dim frmSS As New frmSkillSel, Skilltype As Integer, skill As Integer, Dice As Integer
   ...
   DB.Execute "UPDATE GameSeq SET Seq = 'F', HostAccept = 0, ClientAccept = 0, Trader = " & CStr(DefenderID) & ", CardID = " & CardID
   Logic.Requery
   ...
   With frmSD
      ...
      Set .imgCrew.Picture = LoadPictureGDIplus(App.Path & "\pictures\" & varDLookup("Picture", "Crew", "CrewID=" & CrewID))
      .Show vbModal
   ...
   Else    'if lost, execute lost bounty orders - kill a crew?
      ...
      End If
   End If
   DB.Execute "UPDATE GameSeq SET Seq = 'R', Trader = 0, CardID = 0"
   Logic.Requery
   'Logic!Seq = "R"
   'Logic!trader = 0
   'Logic.Update
...
Dim frmSD As frmShowdown, skill As Integer, Dice As Integer, winShowdown As Boolean, CardID As Integer
   Logic.Requery
   CardID = Logic!CardID
   ...
   With frmSD
      ...
      Set .imgCrew.Picture = LoadPictureGDIplus(App.Path & "\pictures\" & varDLookup("Picture", "Crew", "CrewID=" & varDLookup("FugitiveID", "ContactDeck", "CardID=" & CardID)))
      .Show vbModal
      winShowdown = .winShowdown
   End With
...
Dim skillcnt, skilldiscards, extraSkill As Integer
            ...
            Else 'you lose :(
               doWorkSkillTest = 3
            End If
            PutMsg player.PlayName & "'s Skill Test: Rolls a " & Dice & " with added " & cstrSkill(WSkill) & " skill points of " & CStr(skillcnt - Dice) & " for a total of " & skillcnt & " to " & IIf(doWorkSkillTest = 0, "succeed :^)", IIf(doWorkSkillTest = 1, "part win", "lose :^(")), player.ID, Logic!Gamecntr, True, getLeader(), getLeader(), 0, 0, 0, Dice, WSkill
...
Public Sub CryBabyLaunch()
   ...
   With Verse.CryBaby
      ...
      CryBabyDriftY = 9
      ' Load the image
      .Picture = LoadPictureGDIplus(App.Path & "\Pictures\CryBaby.bmp")
      .TransparentColor = 0
      ...
      ' Center on ship, adjusted for cargo bay
      .Left = Verse.Imag(player.ID).Left + (Verse.Imag(player.ID).Width \ 2) - (.Width \ 2)
      .Top = Verse.Imag(player.ID).Top + (Verse.Imag(player.ID).Height * 0.65) - (.Height \ 2)
