...
Dim frmJoSel As frmJobSel
   ...
   With frmAction
      ...
      If (Not .buydone) And (Not onlyFullburn) And Not reaverActive Then  ' Buy and Shore leave *may* be active
         ...
         .chkShore.Enabled = (Nz(varDLookup("SupplyID", "Supply", "SectorID=" & SectorID), 0) > 0 Or hasShipUpgrade(player.ID, 19)) And hasDisgruntled(player.ID) And doShoreLeave(player.ID, True) <= getMoney(player.ID)
