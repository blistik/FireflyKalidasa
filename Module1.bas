...
Public Type TokenAnimType
    ...
    durationMs As Long
    Active As Boolean
    PulseActive As Boolean
    PulseStartTime As Double
    PulseDurationMs As Long
    OrigW As Long
    OrigH As Long
End Type

Public TokenAnim(1 To 159) As TokenAnimType   ' plenty for all sectors
Public ATokenAnim(1 To 159) As TokenAnimType
...
Public ReaverFlageActive(1 To 4) As Boolean 'enable the Reaver-Flag effects for the Imag()s
...
On Error Resume Next
  ...
  ElseIf Left(Command$, 16) = "Provider=MSDASQL" Then
     ...
     datab = ConStr
  Else
     datab = Command$   'D:\Progs\GitHub\FireflyKalidasa\FireflyKalidasaSOLO.mdb
     ConStr = "Provider=Microsoft.JET.OLEDB.4.0;Data Source=" & datab & ";Persist Security Info=False"
  End If
...
Dim X
   ...
   DB.Execute "UPDATE Players SET Name = Null, Seq=0, SectorID = Null, Leader=0, Pay = 0, Warrants=0, Goals = 0, Fuel = 0, Parts = 0, Cargo = 0, Contraband = 0, " & _
              ...
              "AI = 0, Solid1 = 0, Solid2 = 0, Solid3 = 0, Solid4 = 0, Solid5 = 0, Solid6 = 0, Solid7 = 0, Solid8 = 0, Solid9 = 0"
   
   DB.Execute "DELETE from Events"
   DB.Execute "DELETE from PlayerSupplies"
   DB.Execute "DELETE from PlayerJobs"
   DB.Execute "DELETE from HavenSupplies"
   ...
   DB.Execute "UPDATE Board Set Token = TokenSet, AToken = 0, Haven = 0"
...
Public Sub initSound()
   ...
   initMciSound "clack"
   initMciSound "Sonar"
   initMciSound "Wormhole"
   initMciSound "Boarded"
   initMciSound "Alarm"
   initMciSound "refresh"
   initMciSound "flakgun"
   initMciSound "deploy"
...
Public Sub closeSound()
   closeMciSound "Alert"
   closeMciSound "Burn"
   closeMciSound "Reaver"
   closeMciSound "Cruiser"
   closeMciSound "Win"
   closeMciSound "yourgo"
   closeMciSound "mosey"
   closeMciSound "beep"
   closeMciSound "no"
   closeMciSound "msg"
   closeMciSound "gear"
   closeMciSound "reload"
   closeMciSound "clack"
   closeMciSound "Sonar"
   closeMciSound "Wormhole"
   closeMciSound "Boarded"
   closeMciSound "Alarm"
   closeMciSound "refresh"
   closeMciSound "flakgun"
   closeMciSound "deploy"
   
End Sub

Public Sub initMciSound(ByVal alias As String)
Dim Path As String
On Error Resume Next

   Path = App.Path & "\sounds\"
   mciSendString "open """ & Path & alias & ".wav"" alias " & alias, vbNullString, 0, 0
   
End Sub

Public Sub playMciSound(ByVal alias As String)
On Error Resume Next
   mciSendString "seek " & alias & " to start", vbNullString, 0, 0
   mciSendString "play " & alias, vbNullString, 0, 0
   
End Sub

Public Sub closeMciSound(ByVal alias As String)
On Error Resume Next
...
'add new sounds to initSound
Public Sub playsnd(bittype)
   ...
   Select Case bittype
       ...
       Case 14
           playMciSound "Sonar"
       Case 15
           playMciSound "Wormhole"
       Case 16
           playMciSound "Boarded"
       Case 17
           playMciSound "Alarm"
       Case 18
           playMciSound "refresh"
       Case 19
           playMciSound "flakgun"
       Case 20
           playMciSound "deploy"
       Case Else
           playMciSound "Beep"
...
Public Sub playsndX(bittype, Optional ByVal sync As Boolean = False)
Dim X As Long
Dim Y As Long
Dim z As Long
Dim Path As String
...
Dim c() As String
   ...
   If playerID > 6 And SectorID <> LastSectorID And LastSectorID > 0 And leaveToken Then    'cutter 7-12
      changeToken LastSectorID, 1, False, playerID    'leave another token behind
      'if dropping a token in this player's sector, then ignore it
      If LastSectorID = getPlayerSector(player.ID) Then
         ignoreToken = LastSectorID
      End If
   End If
   ...
   If syncsound Then
      If sound > 0 Then
         playsndX sound, syncsound
      Else
         Select Case playerID
         Case 1 To 4
            playsndX 1, syncsound
         Case 5, 6
            playsndX 4, syncsound
         Case Else
            playsndX 3, syncsound
         End Select
      End If
      DoEvents
   End If
...
Public Sub StartTokenMove(ByVal SectorID As Integer, ByVal StartX As Long, ByVal StartY As Long, ByVal targetX As Long, ByVal targetY As Long, ByVal durationMs As Long)
    With TokenAnim(SectorID)
        .SectorID = SectorID
        .StartX = StartX
        .StartY = StartY
        .targetX = targetX
    ...
    Main.tmrTokenAnim.Enabled = True
End Sub

Public Sub StartATokenMove(ByVal SectorID As Integer, ByVal StartX As Long, ByVal StartY As Long, ByVal targetX As Long, ByVal targetY As Long, ByVal durationMs As Long)
    With ATokenAnim(SectorID)
        .SectorID = SectorID
        .StartX = StartX
        .StartY = StartY
        .targetX = targetX
    ...
    Main.tmrATokenAnim.Enabled = True
End Sub
   ...
   If X > 0 Then 'we got company!
      ...
      If G > 0 Then 'WhisperX1
         If MessBox("The Xunsu Whisper X1 should outrun your contact, do you want to give it a burn?", "Sector Contact", "Yes", "No", 0, 0, 14) = 0 Then
            ...
            If Dice > 3 Then
               checkWhisperX1 = True
               PutMsg player.PlayName & " fired up the Xunsu Whisper X1 to full power, now EVADE", player.ID, Logic!Gamecntr, True, 0, 0, 14, 0, 0, Dice
            Else
...
Public Sub changeToken(ByVal SectorID As Integer, ByVal cnt As Integer, Optional ByVal sound As Boolean = True, Optional ByVal playerID As Integer = 0)
    ...
    Dim X As Integer
    Dim tokenCnt As Integer
    Dim fromX As Long, fromY As Long
    Dim coords As String, c() As String

    tokenCnt = varDLookup("Token", "Board", "SectorID=" & SectorID)

    ' Removing tokens (cnt < 0)
    If tokenCnt + cnt < 0 Then
        DB.Execute "UPDATE Board SET Token = 0 WHERE SectorID = " & SectorID
        Main.displayToken SectorID, 0
        Exit Sub
    End If

    ' Only add tokens if < 6
    If tokenCnt < 6 Then
        ...
        ' Update static display immediately
        Main.displayToken SectorID, X

        ' -------------------------------
        ' ANIMATION ORIGIN DECISION
        ' -------------------------------

        If playerID <> 0 Then
            ' Animate from the Reaver ship itself
            fromX = Main.Verse.Imag(playerID).Left
            fromY = Main.Verse.Imag(playerID).Top
        ...
        ' -------------------------------
        ' START THE REAVER TOKEN ANIMATION
        ' -------------------------------
        StartTokenMove SectorID, fromX, fromY, Main.Verse.imgToken(SectorID).Left, Main.Verse.imgToken(SectorID).Top, 3000                                                                          ' duration (tune later)
...
Public Sub changeAToken(ByVal SectorID As Integer, ByVal cnt As Integer, Optional ByVal sound As Boolean = True)
    ...
    ' If removing tokens (cnt < 0)
    If tokenCnt + cnt < 0 Then
        DB.Execute "UPDATE Board SET AToken = 0 WHERE SectorID = " & SectorID
        Exit Sub
    End If

    ' Only place tokens if:
    ' - Sector is not a Haven
    ' - TokenSet = 0 (i.e., tokens allowed)
    ' - Token count < 6
    If getHaven(SectorID) = 0 _
       And varDLookup("TokenSet", "Board", "SectorID=" & SectorID) = 0 _
       And tokenCnt < 6 Then
        ...
        ' Only animate if the Cruiser is not already in this sector
        If cruiserSectorID <> SectorID Then

            ' NEW: Use the actual Cruiser image position instead of DB Slot5
            fromX = Main.Verse.Imag(5).Left
            fromY = Main.Verse.Imag(5).Top + 680   ' your visual offset
            ...
            With Main.Verse.imgAToken(SectorID)
                ...
                ' Start the animation
                StartATokenMove SectorID, fromX, fromY, .Left, .Top, 3000
            End With
...
End Function
Public Function useHavenStorage(ByVal StoryID) As Boolean
   
   useHavenStorage = (varDLookup("HavenStorage", "Story", "StoryID=" & StoryID) = "1")
...
'return the playerid of specific Ship in the sector
Public Function isShipHere(ByVal playerID, ByVal SectorID) As Integer
      isShipHere = Nz(varDLookup("PlayerID", "Players", "PlayerID = " & playerID & " AND SectorID=" & SectorID), 0)
End Function
...
Public Function isMoveCutterEnabled() As Boolean
   isMoveCutterEnabled = (varDLookup("MoveCutter", "Story", "StoryID=" & Logic!StoryID) = 1)
End Function
...
Dim SQL, X, Y, cnt As Integer, SectorID As Integer, ContactID As Integer
   ...
   If Not rst.EOF Then
      ...
      'load up excluded crew if there is room - must reain the last test before reaching success
      If goaldone And Nz(rst!AddCrew) <> "" Then
         goaldone = goalAddCrew(playerID, rst!AddCrew)
      End If
      ...
      If goaldone Then
         ...
         If rst!chngInCutters <> 0 Then
            ...
            NumOfReavers = NumOfReavers + rst!chngInCutters
            If NumOfReavers < 1 Then NumOfReavers = 1
            If NumOfReavers > 6 Then NumOfReavers = 6
            
            For X = 7 To 12
               If X <= NumOfReavers + 6 Then 'active cutter
                  'if not on map, put it there
                  If Nz(varDLookup("SectorID", "Players", "PlayerID=" & X), 0) = 0 Then
                     doMoveCutterPlanetary X
                     'Main.Imag(X).Animate2.StartAnimation
                  End If
               Else   ' inactive
                  Main.Verse.Imag(X).Visible = False
                  DB.Execute "UPDATE Players set SectorID = NULL WHERE PlayerID =" & X
               End If
            Next X
         ...
         If rst!clearAlliance = 1 Then
            DB.Execute "UPDATE Board SET AToken = 0"
         End If
         ...
         If ContactID = 0 Then
            'pass
         ElseIf ContactID = 5 And hasWarrant(playerID) Then
            'no solid
         Else
            SQL = "UPDATE Players SET "
            SQL = SQL & " Solid" & ContactID & "=1 "  'setting SOLID with the Contact
            PutMsg player.PlayName & " is solid with " & varDLookup("ContactName", "Contact", "ContactID=" & ContactID), playerID, Logic!Gamecntr, True, , , , ContactID
...
On Error GoTo err_handler
   ...
   If force Then
      ...
      End If
      If skill > 0 Then
         .picDice(2).Visible = True
         .picDice(2).Picture = LoadPictureGDIplus(App.Path & "\pictures\" & picSkill(skill) & ".bmp") 'LoadPicture(App.Path & "\pictures\D6.jpg")
         .picDice(2).TransparentColor = 0
      End If
...
Public Function InputBoxx(ByVal msg As String, ByVal title As String, Optional DefaultVal As String = "0", Optional ByVal CrewID As Integer = 0, Optional ByVal GearID As Integer = 0, Optional ByVal ShipUpgradeID As Integer = 0, Optional ByVal ContactID As Integer = 0) As Integer
Dim frmPop As frmInputBox
On Error GoTo err_handler
  
   Set frmPop = New frmInputBox
   With frmPop
      .lblMsg = msg
      .Caption = title
      .txtVal.Text = DefaultVal
      If CrewID > 0 Or GearID > 0 Or ShipUpgradeID > 0 Or (ContactID > 0 And ContactID < 10) Then
         .Width = 10320
         .Height = 5040
         .lblMsg.Height = 1600
         .cmd(0).Top = 1900
...
err_handler:
   MsgBox "InputBoxx Error: " & vbCrLf & Err.Description
   Resume normal_exit
...
Dim SQL As String, last As Integer   ', MP
Dim rst As New ADODB.Recordset
...
On Error GoTo err_handler
   ...
   Case "story"
      ...
      While Not rst.EOF
         ...
         If last < cbo.ItemData(cbo.NewIndex) Then
            last = rst!StoryID
         End If
         rst.MoveNext
      Wend
...
Public Function varDLookup(ByVal vstrField As String, ByVal vstrDomain As String, Optional ByVal vstrCriteria As String = vbNullString, Optional ByVal alias As String = vbNullString) As Variant
 ...
 Dim rstLookup As ADODB.Recordset
  ...
  rstLookup.CursorLocation = adUseServer
  
  'Open the selected record
  rstLookup.Open strSQL, DB, adOpenStatic, adLockReadOnly
...
Dim SQL
   getPlayerJobs = 0
   SQL = "SELECT Count(PlayerJobs.CardID) AS Cnt FROM PlayerJobs INNER JOIN ContactDeck ON PlayerJobs.CardID = ContactDeck.CardID WHERE ContactDeck.ContactID > 0 AND ContactDeck.ContactID < 10 AND PlayerID = " & playerID
...
'return the max value set on a Gear feature that is held by a Crew
Public Function getGearAttrib(ByVal CardID, ByVal colName As String) As Variant
Dim rst As New ADODB.Recordset
Dim SQL
   getGearAttrib = 0
   SQL = "SELECT Gear." & colName
   SQL = SQL & " FROM Gear INNER JOIN SupplyDeck ON Gear.GearID = SupplyDeck.GearID "
   SQL = SQL & "WHERE SupplyDeck.CardID = " & CardID
   rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
   If Not rst.EOF Then
      getGearAttrib = rst.Fields(colName)
   End If
   rst.Close
   Set rst = Nothing
End Function
...
Public Function picSkill(ByVal skill) As String
   Select Case skill
   Case 1
      picSkill = "fight"
   Case 2
...
Public Function haveCrewWho(ByVal CrewID) As String
Dim rst As New ADODB.Recordset
Dim SQL
   'may need to manage "On Job" status
   SQL = "SELECT Players.Ship"
   SQL = SQL & " FROM Players INNER JOIN (PlayerSupplies INNER JOIN SupplyDeck ON PlayerSupplies.CardID = SupplyDeck.CardID) ON Players.PlayerID = PlayerSupplies.PlayerID"
   SQL = SQL & " Where SupplyDeck.CrewID = " & CrewID
   rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
   If Not rst.EOF Then
         haveCrewWho = rst!ship
   End If
   rst.Close
   Set rst = Nothing
End Function
...
Public Function getPerkAttributeCrewValue(ByVal playerID, ByVal Attrib As String, Optional ByVal CardID As Integer = 0, Optional ByVal CrewID As Integer = 0, Optional ByVal exclude As Boolean = False) As Integer
Dim rst As New ADODB.Recordset
Dim SQL
   If Attrib = "" Then Exit Function
   'may need to manage "On Job" status
   ...
   rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
   If Not rst.EOF Then
       getPerkAttributeCrewValue = rst.Fields(Attrib)
   End If
   rst.Close
   Set rst = Nothing
End Function
...
'return CardID of a player's crew's *any* gear attrib
...
Dim SQL
   ...
   SQL = SQL & "Where PlayerSupplies.playerID = " & playerID & " And PlayerSupplies.CrewID = " & CrewID & " AND Gear." & Attrib & " <> 0 "
   SQL = SQL & "ORDER BY SupplyDeck.CardID"
   
   rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
   If Not rst.EOF Then
       hasCrewGearAttribute = rst!CardID
   End If
   rst.Close
   Set rst = Nothing
End Function
...
Public Function discardGearOnUse(ByVal GearID As Integer) As Boolean
   If GearID = 0 Then Exit Function
   discardGearOnUse = varDLookup("Discard", "Gear", "GearID =" & GearID)

End Function

Public Function hasShipUpgradeAttribute(ByVal playerID, ByVal Attrib As String) As Integer
Dim rst As New ADODB.Recordset
Dim SQL
   SQL = "SELECT ShipUpgrade.ShipUpgradeID "
   SQL = SQL & "FROM ShipUpgrade INNER JOIN (PlayerSupplies INNER JOIN SupplyDeck ON PlayerSupplies.CardID = SupplyDeck.CardID) ON ShipUpgrade.ShipUpgradeID = SupplyDeck.ShipUpgradeID "
   SQL = SQL & "WHERE PlayerSupplies.PlayerID=" & playerID & " AND ShipUpgrade." & Attrib & " <> 0"
   
   rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
   If Not rst.EOF Then 'find the first one, as could be multi (eg.Cry Baby)
       hasShipUpgradeAttribute = rst!ShipUpgradeID
   End If
   rst.Close
   Set rst = Nothing
End Function
...
Dim SQL
   ...
   While Not rst.EOF
      ...
      DB.Execute "UPDATE SupplyDeck SET Seq =" & DISCARDED & " WHERE CardID = " & rst!CardID
...
Dim SQL, X
   ...
   While Not rst.EOF
      ...
      X = getPerkAttributeCrewValue(playerID, skill, rst!CardID)
      ...
      Else 'koshized rules apply, with exceptions
         If hasGear(playerID, 37, rst!CrewID) And LCase(skill) = "fight" Then  'Jayne's Holdout Pistol
            getSkill = getSkill + 1
         End If
         If hasGear(playerID, 61, rst!CrewID) And LCase(skill) = "fight" Then  'Dobson's Vector Pistol
            getSkill = getSkill + 1
         End If
      End If
...
Dim SQL, leader, CardID, CrewName As String, Disgruntled As Integer, Fendris As Boolean, FendrisUsed As Boolean
   ...
   'see if Leader is receiving a 2nd disgruntle -all crew fired!
   leader = varDLookup("Leader", "Players", "PlayerID=" & playerID)
   'if doing only the leader, check for Fendris and switch
   Fendris = hasCrew(playerID, 26)

   
   SQL = "SELECT Crew.*, SupplyDeck.CardID "
   SQL = SQL & "FROM (Players INNER JOIN PlayerSupplies ON Players.PlayerID = PlayerSupplies.PlayerID) INNER JOIN (Crew INNER JOIN SupplyDeck "
   SQL = SQL & "ON Crew.CrewID = SupplyDeck.CrewID) ON PlayerSupplies.CardID = SupplyDeck.CardID "
   SQL = SQL & "WHERE Players.PlayerID=" & playerID  'PlayerSupplies.OffJob = 0 AND
      
   'Leader check to see if all fired
   ...
   Else
      ...
      If mode = 7 Then 'disgruntled everyone except Leader
         SQL = SQL & " AND Crew.Leader=0"
         mode = 2 'all from here on out
      End If
         ...
         Case 1, 2, 4, 7 'moral only / all
            If rst!Moral = 1 Or mode >= 2 Then 'they are either disgruntled or leaving
               ...
               Else 'add a Disgruntled
                  DB.Execute "UPDATE Crew SET Disgruntled = 1 WHERE CrewID = " & CrewID
               End If
            End If
...
Dim SQL
   ...
   rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
   If Not rst.EOF Then
      DB.Execute "UPDATE SupplyDeck SET Seq = " & IIf(rst!SupplyID > 0, DISCARDED, "0") & " WHERE CardID = " & rst!CardID
...
Dim result As Integer, X As Integer, CrewID, Dice As Integer, mess As String
   ...
   Dice = (RollDice(6) + X)
   If mess = "" Then
      Dice = 0  'dice roll does not apply
   ElseIf mess <> "" And Dice > 4 Then
...
'update their pile status - 0 removed, 5 -discarded
Public Sub doHavenCrew(ByVal CardID, Optional ByVal status As Variant = 0)
Dim CrewID

   CrewID = varDLookup("CrewID", "SupplyDeck", "CardID=" & CardID)
   
   DB.Execute "UPDATE SupplyDeck SET Seq = " & status & " WHERE CardID = " & CardID
   If status > 0 Then   'load
      DB.Execute "INSERT INTO PlayerSupplies (PlayerID, CardID) VALUES (" & player.ID & ", " & CardID & ")"
      DB.Execute "DELETE FROM HavenSupplies WHERE CardID = " & CardID
   Else                 'offload
      'remove any Gear first
      DB.Execute "UPDATE PlayerSupplies SET CrewID = 0 WHERE CrewID = " & CrewID
      'delete the card to the players deck
      DB.Execute "DELETE FROM PlayerSupplies WHERE CardID = " & CardID
      'add them to the HavenSupplies
      DB.Execute "INSERT INTO HavenSupplies (PlayerID, CardID) VALUES (" & player.ID & ", " & CardID & ")"
   End If
   
End Sub

'use for linked, unlinked Gear and any ShipUpgrade
Public Sub doHavenGear(ByVal CardID, Optional ByVal status As Variant = 0)
...
Dim rst As New ADODB.Recordset
Dim SQL, pay As Integer, CryBaby As Integer, CardID As Integer, GearID As Integer  ', DisCardID As Integer
Dim crewcnt As Integer, allcrew As Integer, stash As Integer, discard As Boolean
Dim frmSeize As frmSeized2
   ...
   rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
   While Not rst.EOF
      CardID = hasCrewGearAttribute(playerID, rst!CrewID, "IgnoreWanted")
...
Dim adjacent, a() As String, X, Y
   ...
   Y = 0
   For X = LBound(a) To UBound(a)
      'replaced (Val(a(X)) < 120 Or Val(a(X)) > 122) And (Val(a(X)) < 156 Or Val(a(X)) > 157)
      If getClearSector(Val(a(X))) <> "" And varDLookup("TokenSet", "Board", "SectorID=" & a(X)) = 0 Then   'no ship in this spot, and not in reaver zones
         Y = 1 'we have at least one possible solution
         If check Then
   ...
   If Y = 1 Then
      Do
         ...
         If X > UBound(a) Then X = UBound(a)
         'replaced (Val(a(X)) < 120 Or Val(a(X)) > 122) And (Val(a(X)) < 156 Or Val(a(X)) > 157)
         If getClearSector(Val(a(X))) <> "" And varDLookup("TokenSet", "Board", "SectorID=" & a(X)) = 0 Then
            MoveShip 6, Val(a(X))
            doMoveCorvetteAdjacent = True
            Exit Do
         End If
...
Dim SQL
   ...
   If Not loadnames Then SQL = SQL & " AND AI=0"
   rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
   While Not rst.EOF
...
Public Function isSolid(ByVal playerID, ByVal ContactID) As Boolean
   ...
   If Nz(ContactID, 0) = 0 Or Nz(ContactID, 0) = 10 Or playerID = 0 Then Exit Function
...
Public Function isOffJob(ByVal CardID As Integer) As Boolean
   isOffJob = (varDLookup("OffJob", "PlayerSupplies", "CardID=" & CardID) = 1)
End Function
...
'move a Ship one sector, preferencing a player ship sector if adjacent.    Return: sectorID of any ship encountered
...
Dim SQL, SectorID As Integer, a() As String, B(1 To 20, 1 To 2) As Integer, c() As String, X, Y, z, adjacent As String, vector As Integer, vector2 As Integer
   ...
   a = Split(adjacent, ",")
   cnt = 0
   SQL = "SELECT * FROM Players WHERE Name IS NOT NULL OR PlayerID between 5 AND " & CStr(NumOfReavers + 6)
   rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
   While Not rst.EOF
      'look at each adjacent Sector and record if there is a Player as a count and B()
      ...
      For X = LBound(a) To UBound(a) 'first step array
         ...
         ElseIf (avoid > 0 And avoid = Val(a(X)) And Y = 0) Then 'take out this start sector from scan, unless a player is there
            a(X) = "-1"
         ' replaced (rst!sectorID < 120 Or rst!sectorID > 122) And (rst!sectorID < 156 Or rst!sectorID > 157)
         ElseIf Val(a(X)) = rst!SectorID And varDLookup("TokenSet", "Board", "SectorID=" & rst!SectorID) = 0 Then     'found one, treat B & R as same
            If rst!playerID < 5 Then ' tag you're it
               cnt = cnt + 1
         ...
         Else 'nothing found, check second step sectors
            ...
            For z = LBound(c) To UBound(c)
               ...
               If avoid > 0 And avoid = Val(c(z)) And Y = 0 Then 'take out this start sector from scan, unless a player is there
                  c(z) = "-1"
               ' replaced (rst!sectorID < 120 Or rst!sectorID > 122) And (rst!sectorID < 156 Or rst!sectorID > 157)
               ElseIf Val(c(z)) = rst!SectorID And varDLookup("TokenSet", "Board", "SectorID=" & rst!SectorID) = 0 Then      'found one, treat B & R as same
                  If rst!playerID < 5 Then ' tag you're it
                     cnt = cnt + 1
...
Dim SQL, B(1 To 20, 1 To 3) As Long, X As Integer, Y As Long, z As Long, adjacent
   ...
   rst.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
   While Not rst.EOF
      'replaced rst!sectorID < 120 Or rst!sectorID > 122
      If (ship = 5 And rst!Zones = "A") Or (ship = 6 And (varDLookup("TokenSet", "Board", "SectorID=" & rst!SectorID) = 0 Or getCorvetteSector() = 123)) Or (ship > 6 And rst!Zones <> "A" And Nz(rst!playerID, 0) < 5) Then 'path rules for the ships
...
Public Function hasAlert(ByVal SectorID) As Boolean
  
      If (varDLookup("Token", "Board", "SectorID=" & SectorID) > 0) Or (varDLookup("AToken", "Board", "SectorID=" & SectorID) > 0) Then
         hasAlert = True
      End If
      
End Function

'return true if an adjacent Sector to the player has an active Alert token present
...
Dim currentSectorID, adjacent, a() As String, X
   ...
   For X = LBound(a) To UBound(a)
      ...
      If (varDLookup("Token", "Board", "SectorID=" & a(X)) > 0) Or (varDLookup("AToken", "Board", "SectorID=" & a(X)) > 0) Then
...
Dim ContactID
   If CardID < 1 Then Exit Function
...
Public Function getPlanetName(ByVal SectorID) As String
Dim rst As New ADODB.Recordset
Dim SQL
   If Nz(SectorID, -1) = -1 Then Exit Function
   SQL = "SELECT Planet.PlanetName, Planet.System "
   SQL = SQL & "FROM Planet "
...
Public Function removeBounty(ByVal CrewID) As Boolean 'out of deck
Dim SQL As String
Dim rst As New ADODB.Recordset

   If Not isBountyEnabled Then Exit Function
   
   SQL = "SELECT Seq "
   SQL = SQL & "FROM ContactDeck "
   SQL = SQL & "Where ContactDeck.ContactID = 10 and FugitiveID = " & CrewID
   rst.Open SQL, DB, adOpenDynamic, adLockOptimistic
   If Not rst.EOF Then
      If rst!Seq = DISCARDED Then 'we need to remove it from faceup cards
         removeBounty = True
         rst.Update "Seq", 0
      ElseIf rst!Seq > DISCARDED Then
         rst.Update "Seq", 0
      End If
   End If
   rst.Close
...
Public Sub doForceFugitives() 'reveal open bounty fugitives in supply deck
Dim rst As New ADODB.Recordset
Dim SQL
