...
Private Const DRAG_THRESHOLD As Single = 60 'twips (~4 pixels)
...
Private Sub cmdDiscard_Click()
   If discardJob(lbl(0).Caption, JobCardID) Then
      Unload Me
   End If
End Sub
...
Dim x
      ...
      If JobCardID < 1 Then
         Exit Sub
      End If
      SQL = "SELECT Contact.Picture, Contact.ContactName, JobType.JobTypeDescr, Profession.ProfessionName, ContactDeck.*, JobType_1.JobTypeDescr AS JobType2 "
      ...
      If Not rst.EOF Then
         ...
         If ContactID = 5 Then 'Alliance Background
         ...
         Set img(1).Picture = LoadPictureGDIplus(App.Path & "\gui-job\Contact" & ContactID & ".bmp")
         img(1).ToolTipText = rst!ContactName
         ...
         If Not IsNull(rst!JobOrder) Then
            ...
            lbl(7).Visible = True
            lbl(7).Caption = Replace(Replace(Replace(rst!JobOrder, ". ", "." & vbNewLine), "/", "/" & vbNewLine), "&", "&&")
            If Len(rst!JobOrder) > 50 Then
               img(5).Left = 5460
               lbl(7).Left = 5850
               lbl(7).Width = 2685
               lbl(8).Width = 5325
         ...
         If rst!Job3ID > 0 Then
            ...
            rst2.Open SQL, DB, adOpenForwardOnly, adLockReadOnly
            If Not rst2.EOF Then
               lbl(7).Caption = lbl(7).Caption & vbNewLine & rst2!JobDesc
               img(5).Left = 6000
               lbl(7).Left = 6390
               lbl(7).Width = 2145
               lbl(7).Height = 1755
