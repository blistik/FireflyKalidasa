...
Dim c() As String
   ...
   If Not rst.EOF Then
      For slot = 1 To 6
         Me.Imag(slot).Visible = Not IsNull(rst!Zones)
      Next slot
      ...
      Me.Imag(5).Visible = (rst!Zones = "A")
      Me.Imag(6).Visible = Not (rst!Zones = "A")
      Me.Imag(5).Tag = rst!Zones
      more = 0
      For slot = 1 To 5
         coords = rst.Fields("Slot" & slot).Value
         If IsNull(coords) Then
         ...
         Else
            ...
            Me.Imag(slot + more).Left = c(0)
            Me.Imag(slot + more).top = c(1)
         End If
      Next slot
   End If
